...
const TrialConfirmationOverlay = ({
    ...
    // Handles the "slam out" animation before triggering the parent unmount functions
    const handleClose = (action) => {
        setIsClosing(true);
        setTimeout(() => {
            if (action === 'confirm') onConfirm();
            else onCancel();
        }, 300); // Matches the modal animation duration
    };
    ...
    return (
        <div className={`modal-backdrop ${isClosing ? 'fade-out' : 'fade-in'}`}>
            {/* Setting a fixed width so the modal doesn't crush the layout */}
            <div className={`modal-content-box ${isClosing ? 'modal-slam-out' : 'modal-slam'}`} style={{ width: '850px', height: 'auto' }}>

                <div className="confirm-modal-layout">

                    {/* MODAL HEADER (Modeled after Review Challenges) */}
